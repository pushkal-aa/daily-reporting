...
const { BQ_PROJECT, START_DATE } = require('./config');
...
function spendQuery() {
  return `SELECT date, vertical, deviceType, publisher, SUM(cost) AS cost
...
function revenueQuery() {
  return `SELECT c.date, c.vertical, c.deviceType, c.partnerId AS partner, ROUND(c.revenue) AS planBucket,
       COUNT(*) AS units, SUM(c.revenue) AS revenue
...
async function fetchRows() {
  const bq = getClient();
  const [spendRows] = await bq.query({ query: spendQuery() });
  const [revRows] = await bq.query({ query: revenueQuery() });
  return {
