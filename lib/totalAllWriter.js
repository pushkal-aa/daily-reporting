...
// Falls back to BigQuery-derived actuals unless a Manual History row
// exists for this vertical+month (used for Feb-May, where BigQuery has
// no spend history at all).
function combinedMonthActuals(data, manualHistory, v, monthKey) {
  const manualKey = v.tab + '|' + monthKey;
  if (manualHistory[manualKey]) return manualHistory[manualKey];
  return monthActuals(data, v.code, monthKey);
}

function buildTotalAllMatrix(data, targets, manualHistory) {
  manualHistory = manualHistory || {};
  const rows = [];

  // Union of months present in BigQuery data and months entered manually,
  // so Feb-May (manual-only) and June+ (BigQuery) both get blocks.
  const monthSet = {};
  listMonthsAcrossVerticals(data).forEach(mk => { monthSet[mk] = true; });
  Object.keys(manualHistory).forEach(key => { monthSet[key.split('|')[1]] = true; });
  ...
  months.forEach(monthKey => {
    ...
    VERTICALS.forEach(v => {
      const actuals = combinedMonthActuals(data, manualHistory, v, monthKey);
  ...
  const ytdLabel = months.length ? monthLabel(months[0]) : START_DATE;
  rows.push(['YTD (since ' + ytdLabel + ')']);
